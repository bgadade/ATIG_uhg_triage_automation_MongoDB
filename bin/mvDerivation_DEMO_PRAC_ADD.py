...
def var_hbpSolelyInHospital(row, cleanHbpVal):
    if row[cleanHbpVal]:
            return row[cleanHbpVal]
    else:
        return ""
...
def var_parseDate(row, ColumnName, coltyp='C', deaNumCol=""):
    ...
    elif row[ColumnName] == "00:00:00":
        return ErrorMessage([], ["R", "Unable to determine the Format", row[ColumnName]])
    else:
        if coltyp == 'C':
            val = row[ColumnName]
            if is_float(val):
...
def var_getNewPatients(row, newPatientsCol):
    if row[newPatientsCol]:
        return row[newPatientsCol]
    else:
        return ""
...
def var_hospAffStatus(row, hospAffStatus):
        hospAffStatusLkp = ''
        if row[hospAffStatus]:
            if len(row[hospAffStatus]) <= 3:
                bestMatch = elastic.gridLookup('hosp_aff_status', row[hospAffStatus], 1,**{'match_fields': ['acronym'], 'operator': 'and', 'restrict': True})
                if bestMatch and bestMatch["acronym"]:
                    hospAffStatusLkp = bestMatch["acronym"]
            else:
                bestMatch = elastic.gridLookup('hosp_aff_status', row[hospAffStatus], 2, **{'match_fields': ['abb'], 'operator': 'and', 'restrict': True})
                if bestMatch and bestMatch["acronym"]:
                    hospAffStatusLkp = bestMatch["acronym"]
            if hospAffStatusLkp:
                return hospAffStatusLkp
            else:
                return ErrorMessage([], ["C", "Could not match Hospital Affiliation status", row[hospAffStatus]])
...
def formatDate(df,argDi):
    inpCol = argDi["inputCol"]
    outCol = argDi["outputCol"]
    if not isinstance(inpCol, list):
        cols = [inpCol]
        outCols = [outCol]
    else:
        cols = inpCol
    ...
    def fmt(x):
        try:
            ...
            return x
    for col, outClm in zip(cols, outCols):
        df[outClm]=df[col].map(lambda x:fmt(x))
    return df
...
def distributeFinalSpec(row, spec, specInd, indList, include = True):
    if include:
        if row[specInd].lower() in indList:
            return row[spec]
    else:
        if row[specInd].lower() not in indList:
            return row[spec]
    return ""
...
def clarifyCorrelatedCols(df, funInp):
    ...
    for idx, row in df.iterrows():
        ctr += 1
        rowBool = [row[x] == "" or row[x].lower() == "none" or row[x] == None or isErrorVal(row[x]) == True for x in correlatedCols]
...
def var_finalInpSpecForBrd(row, sotExSpec, specCol):
    if row[sotExSpec]:
        return row[sotExSpec]
    else:
        return row[specCol]

def var_masterSpec(row, colname,spec, inSot):
    row = row.fillna('')
    if len([x for x in row[colname].split("|") if x]) > 0:
        return row[colname].split("|")[0]
    else:
...
def associateBrdWithSpec(df,argDi):
    diDf=argDi['diDf']
    joinCols=argDi["joinCols"]
    brdToSpecColMapping=argDi["brdToSpecColMapping"]
    mergeIndCol=argDi["mergeIndCol"]
    clrMsg=argDi["clrMsg"]
    clrCols=argDi["clrCols"]
    brdDfName=argDi["brdDfName"]
    colsToCheckBrdInSpec=argDi["colsToCheckBrdInSpec"]
    brdRowsForMrg=argDi["brdRowsForMrgCol"]
    brdDf=diDf[brdDfName]
    brdDf=brdDf.loc[brdDf[brdRowsForMrg]=='Y']
    if brdDf.empty:
        return df
    mrgdDf=pd.merge(df,brdDf,left_on=joinCols["left_on"],right_on=joinCols["right_on"],how='outer',indicator=mergeIndCol,suffixes=('','_y'))
    #mrgdDf=mrgdDf.replace('nan','')
    def func(row,clrCols,clrMsg,inpStr,brdToSpecColMapping):
        if inpStr=='right_only':
            for col in clrCols:
                row[col]=ErrorMessage([], ["c", clrMsg,row[col]])
            for brdCol,specCol in list(brdToSpecColMapping.items()):
                row[specCol]=row[brdCol]
        elif inpStr=='both':
            for brdCol,specCol in list(brdToSpecColMapping.items()):
                row[specCol]=row[brdCol]
        return row

    finalDf=mrgdDf.apply(lambda row:func(row,clrCols,clrMsg,row[mergeIndCol],brdToSpecColMapping),axis=1)
    return finalDf[df.columns].fillna('')


def gender(row, genderCol, errMsg):
    if not row[genderCol]:
        return ErrorMessage([], ["c", errMsg, row[genderCol]])
    else:
        return row[genderCol]
...
def getDirInd(df,argDi):
    ...
    return df
'''
def getCliaNum(row,cliaNum,offLab):
    cliaNum = row[cliaNum]
    if row[offLab].lower() == "y":
        if cliaNum:
            return cliaNum
        else:
            return ErrorMessage([], ["C", "CLIA Number not found ", ""])
...
def var_directMap(row, col):
    if row[col]:
        return row[col]

def var_getPcpOBPatients(row, primspec, provType, pcpOBGynNewPat):
    # if row[pcpOBGynNewPat]:
    #     return row[pcpOBGynNewPat]

    if row[provType].lower() == "pcp" or row[primspec].lower() == "obstetrics and gynecology" or isErrorVal(row[provType]) or not row[primspec]:
        if row[pcpOBGynNewPat]:
            return row[pcpOBGynNewPat]
        elif row[provType].lower() == "pcp":
            return "Y"
        elif row[primspec].lower() == "obstetrics and gynecology" and row[provType].lower() == "hosp" or "spec":
            return "N"
        else:
            return ErrorMessage([], ["c", "Mandatory Field Missing", ""])
    else:
        return ""

def cleanDirInd(df,argDi):
    return cleanAddressDir(df, argDi)
'''
...
def getHandicapAccessTypeInd(df, argDi):
    ...
    return df
'''
def getHBPinHospOnlyValue(df, argDi):
    df = df.fillna('')
    col1 = argDi['inputCol1']
    col2 = argDi['inputCol2']
    mappingCol = argDi["mappingCol"]
    outCol = argDi['outputCol']
    colMap = argDi['colMap']
    colMapVal=eval(df[colMap][0])
    df[outCol] = ''
    if mappingCol in list(colMapVal.keys()):
        df[outCol]=df[col1]
    else:
        df[outCol] = df[col2]
    return df

def var_hospAffLenValidateWithDegFilter(df, funcInp):
    """
        modified validate_length function from mv_common. This function will iterate over each row and check length
        of string w.r.t business rule like string should be X character long or equal to.
        inputs:-
            df:- dataframe from masterDict
            funcInp:- list of required parameter
        returns:-
            df:- inplace changed dataframe
    """
    df=df.fillna('')
    col, size, type, excepCol = funcInp['params']
    allowed_degrees = funcInp["standardsKeys"][0]
    mandatory = eval(funcInp.get('mandatory', 'True'))
    ...
    for idx, row in df.iterrows():
        ...
        if row["var_getDegreeCredTitle"] not in constants.standardsDict[allowed_degrees]:
            df.loc[idx, excepCol] = row[col]
