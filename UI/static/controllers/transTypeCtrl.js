angular.module("myapp")
  .controller("TransTypeController", function ($scope, mainService, $state, $location) {
    ...
    $scope.dropSuccessHandler = function ($event, selectedItems, array, selectedRow) {
      ...
      if (selectedItems.length != 0) {
        for (var i = 0; i < selectedItems.length; i++) {
          console.log(array[selectedItems[i]].TransactionType, $scope.allTypes[array[selectedItems[i]].TransactionType]);
          if ($scope.allTypes[array[selectedItems[i]].TransactionType] > 0) {
            console.log(">000");
            //   $scope.allTypes[array[selectedItems[i]].TransactionType] -= 1;
            $scope.allTypes[array[selectedItems[i]].TransactionType] -= 1;
            console.log($scope.allTypes[$scope.selectedType]);
          }
          // else {
          //   console.log("else NaN");
          //   console.log($scope.allTypes[array[selectedItems[i]].TransactionType]);
        ...
        $scope.allTypes[$scope.selectedType] += selectedItems.length;
      }
      $scope.selectedArray = [];
      $scope.selecteddelArray = [];
      $scope.selectedRowItem = [];
    ...
    $scope.highlightDragRow = function (rowIndex, updatedrowIndex, selectedArray, sheetData, data) {
      console.log(data, rowIndex, updatedrowIndex, selectedArray);
    ...
    $scope.cloneSelectedRow = function () {
      ...
      // $scope.selectedSheetData = { Sheet1: [5] } //  for error handling check
      let cloneData = {
        "cloneflag": true,
    ...
    $scope.deleteSelectedRow = function () {
      ...
      if (clonedArray) {
        console.log($scope.selectedRowItem)
        // let selectedRow = $scope.selectedRowItem.some(ele => (ele.row == 'Original'))
        console.log($scope.selectedRowItem.every(ele => (ele.row == 'Cloned')));
        let selectedRow = $scope.selectedRowItem.every(ele => (ele.row == 'Cloned'))
        console.log(clonedArray, selectedRow, "clonedArray if", selectedRow.length);
        if (selectedRow) {
          ...
          $scope.error = "Original rows can't be deleted.Please select cloned rows to delete";
          // alert("Original rows can't be deleted.Please select cloned rows to delete");
          let options = {
            backdrop: true,
            Keyboard: false
          }
          $('#errorModal').modal('show', options);
        }
    ...
    $scope.toggleAll = function () {
      ...
      if ($scope.selectAll) {
        ...
        if ($scope.cloneData.length > 0 && $scope.resetData) {
          ...
          $scope.selectAll = false;
          $scope.error = "Please save or reset the changes to select all rows";
          let options = {
            backdrop: true,
            Keyboard: false
          }
        ...
        if (response == 'success') {
          ...
          mainService.fetchSotData(function (response) {
            ...
            if (response.data == 'Select only clonned rows') {
              ...
              $scope.error = 'Select only cloned rows';
              // alert("Original rows can't be deleted.Please select cloned rows to delete");
              let options = {
                backdrop: true,
                Keyboard: false
              ...
              $scope.error = 'Select atleast one cloned row to declone';
              // alert("Original rows can't be deleted.Please select cloned rows to delete");
              let options = {
                backdrop: true,
                Keyboard: false
